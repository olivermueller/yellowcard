...
pitch at the end of H1), trim the control outcome distribution within
cells (both directions), and re-run the DML per side.

Efficiency: the propensity and only the bounded DV's outcome nuisance are
refit on each trimmed sample (the other DVs' residuals are not needed).

Output: data/lee_bounds_windows.csv (one row per window x DV, ATE + bounds).
"""
...
def crossfit_one(d, W, dv):
    """Residualise T and a single DV (propensity + one outcome nuisance)."""
...
def main():
    ...
    out.to_csv("data/lee_bounds_windows.csv", index=False)
    print("\n=== bounds across windows (relative) ===")
    print(out.pivot(index="window", columns="dv", values=["rel", "lee_lo_rel", "lee_hi_rel"]).to_string())
    print("\nwrote data/lee_bounds_windows.csv")
