"""Binarized-outcome conditional Lee bounds, all H2 windows (backlog C).

Same design as build_lee_windows.py (corrected sample, cell-based trims:
position x pre-activity tercile) but outcomes are binarized: P(any foul),
P(any pressure), P(any tackle), P(any defensive action) in the window.
Rationale: Lee trimming bounds are tightest for bounded outcomes — deleting
a p-share of controls can move a probability by at most ~p, whereas on
right-skewed counts the same trim moves the mean by a multiple of p. If the
count-based bounds cross zero for arithmetic reasons only, these should not.

Output: data/lee_bounds_binary.csv
"""
...
def crossfit_bin(d, W, ycols):
    """Residualise T and the given binary outcome columns."""
...
def main():
    ...
    out.to_csv("data/lee_bounds_binary.csv", index=False)
    print("\n=== zero-excluding bounds? ===")
    print(out.pivot(index="window", columns="dv", values="zero_excluded").to_string())
    print("\nwrote data/lee_bounds_binary.csv")
