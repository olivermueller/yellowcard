...
## A. Story & framing
- [x] **§2 Related work FINAL (2026-09-10, Oliver's edit 58bd623):** two-line structure (team outcomes; cross-match deterrence); referee strand dropped by design (reversed causal arrow — card as outcome — and we do not control for referees; sensitivity analysis covers referee leniency); Durygin critique folded into line 1; basketball paragraph closes with forward reference to §3. Compiles clean, 30 pp, zero bib warnings (uncited referee entries stay in references.bib). Originally: **§2 Related work WRITTEN (2026-07-31, commit 62ffdcf)** on a re-run, verified literature search: three strands + scoped gap (causal/player-level/within-match, football; Durygin et al. 2024 StatsBomb descriptive work distinguished; NBA foul-trouble literature acknowledged; Badiella et al. 2023 names our question as open). Corrections vs old notes: Červený = Empirical Economics; Deutscher 2013 → Deutscher et al. 2022 + Kruse/Maennig WP; Yellow fever = Philipson JRSS-A 2026. OPEN: Oliver reviews §2.
- [x] **§1 Introduction WRITTEN (2026-09-10, manuscript 5813cbf)** per Oliver's 9-sentence formula (hook/gap/study/conclusion/outline), drafted and revised in chat, inserted and pushed. STILL OPEN from this item: abstract (~200 words) + keywords. Original: **(P2)** Write abstract (~200 words) + §1 Introduction (lead with the football/behaviour question; three margins: booked players withdrawn 2.7x more often [non-causal phrasing], player fouls −26.5%, teammates +4.5% fouls / +3.6% recoveries) + keywords (3–6, not in title).
...
- [x] **Women's-league replication CONSIDERED and REJECTED (Oliver, 2026-07-31):** Hudl/StatsBomb released five complete 2023/24 women's league seasons (WSL, Liga F, Frauen-Bundesliga, Serie A Women, NWSL 2023; 771 matches). Verified (fetch-level): football-data.co.uk carries NO women's leagues; complete 1X2 odds exist on BetExplorer/OddsPortal for all five seasons but scraping is contractually prohibited (Livesport ToS); The Odds API history starts 2026-02; Betfair = paid, coverage unverified. Age would have been solvable (Wikidata, ~1,500 players). Decision: stick with the male sample; keep this record for reviewer responses on generalizability.
...
- [ ] **(P3)** Reviewer-response ammunition: referee-FE robustness run (referee identity is pre-treatment; StatsBomb match metadata likely carries referee for our competitions — verify) if a reviewer asks why referees are not controlled; current stance = referee leniency as unobserved confounder bounded by the RV analysis (§5.4.1), partially absorbed by pre-window counts.
