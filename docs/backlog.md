...
## D. Outcomes / targets
- [~] **(P1)** **Extended outcome families (Oliver, 2026-07-29):** Family 1 = defensive family extended by ball recoveries, clearances, blocks, interceptions (added to pressures/tackles/fouls); Family 2 (new) = miscontrols, dispossessions, times dribbled past. Family names to be settled (proposed: "defensive actions" / "adverse individual outcomes"). Rate-based targets DROPPED (post-treatment denominator selection). Literal StatsBomb "Error" event unusable (defined as mistake leading to a shot; 0.003/player-match). Estimation on the primary window running (`src/build_outcome_families.py` → `data/outcome_families.csv`); manuscript integration after Oliver reviews estimates + names.
