...
## D. Outcomes / targets
- [~] **(P1)** **TAXONOMY (Oliver, 2026-07-30): opponent-directed (pressures, tackles, fouls; sanction risk) vs ball-directed (recoveries, clearances, blocks, interceptions; no sanction path) — category aggregates replace the 7-component grand total (NOT shown).** Pipeline rewired (commit 4a25240), full chain re-running; manuscript integration (taxonomy in §3.3 with rationale, two-panel tables, 5x2 window figure, §5 text on the category contrast) next. Expected headline: opponent-directed −6.1%*** vs ball-directed ≈ 0.
