...
## D. Outcomes / targets
- [~] **(P1)** **Extended outcome families — INTEGRATED (2026-07-29, manuscript commit 211558e):** full chain re-run on the 7-component defensive-actions aggregate (−4.3%*** primary; −4.4..−5.5% across windows; heterogeneity null p=.56; direct effect −4.1%***; RV 1.8/0.9%; ladder −0.219..−0.231); adverse events in §3.3 + Table 3 (aggregate −5.6% p=.024, miscontrols −7.9% p=.031, dispossessions/dribbled-past null — no hesitancy channel); binary bounds def-action rows updated; Appendix A/C refreshed; 29 pp. OPEN: Oliver reviews the integrated version; then the conditional cleanup below. Family 1 = defensive family extended by ball recoveries, clearances, blocks, interceptions (added to pressures/tackles/fouls); Family 2 (new) = miscontrols, dispossessions, times dribbled past. Family names SETTLED (Oliver): **"defensive actions"** / **"adverse events"**. Rate-based targets DROPPED (post-treatment denominator selection). Literal StatsBomb "Error" event unusable (defined as mistake leading to a shot; 0.003/player-match). Estimation on the primary window done (`src/build_outcome_families.py` → `data/outcome_families.csv`); full-chain re-run on the new aggregate in progress; manuscript integration next.
